...
object CollectionHelper {
    ...
    fun clearImagesFolder(context: Context, podcast: Podcast) {
        // clear image folder
        val imagesFolder: File = File(context.getExternalFilesDir(""),
            FileHelper.determineDestinationFolderPath(Keys.FILE_TYPE_IMAGE, podcast.name)
        )
    ...
    fun deletePodcastFolders(context: Context, podcast: Podcast) {
        ...
        // delete audio folder
        val audioFolder: File = File(context.getExternalFilesDir(""),
            FileHelper.determineDestinationFolderPath(Keys.FILE_TYPE_AUDIO, podcast.name)
        )
    ...
    fun exportCollectionOpml(context: Context, podcastList: List<Podcast>) {
        ...
        CoroutineScope(IO).launch {
            FileHelper.backupCollectionAsOpmlSuspended(
                context,
                podcastList
            )
        }
            ...
            for (i in numberOfEpisodes -1 downTo numberOfAudioFilesToKeep) {
                ...
                if (canBeDeleted(episode)) {
                    ...
                    try {
                        ...
                        LogHelper.e(
                            TAG,
                            "Unable to delete file. File has probably been deleted manually. Stack trace: $e"
                        )
                    }
