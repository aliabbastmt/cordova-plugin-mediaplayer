...
import com.radiobase.radiobase.R
...
import com.tmt.mediaplayer.extensions.copy
...
object DownloadHelper {
        ...
        ioScope.launch {
            ...
            for (i in uris.indices) {
                ...
                val deferred: Deferred<String> = async {
                    NetworkHelper.resolveRedirectsSuspended(
                        uris[i].toString()
                    )
                }
    ...
    private fun enqueuePodcastMediaFiles(context: Context, podcast: Podcast, episodes: List<Episode>, isNewPodcast: Boolean, ignoreWifiRestriction: Boolean = false) {
        ...
        if (ignoreWifiRestriction || PreferencesHelper.loadEpisodeDownloadOverMobile() || NetworkHelper.isConnectedToWifi(
                context
            )
        ) {
            ...
            if (episodes.isNotEmpty()) {
                ...
                if (episodes.size >= Keys.DEFAULT_NUMBER_OF_EPISODES_TO_KEEP) {
                    ...
                    if (oldestEpisode.audio.isNotBlank()) {
                        ...
                        collectionDatabase.episodeDao().update(
                            CollectionHelper.deleteEpisodeAudioFile(
                                episode = oldestEpisode,
    ...
    private fun addPodcast(context: Context, rssPodcast: RssHelper.RssPodcast) {
        ...
        if (oldPodcast != null) {
            ...
            episodes = CollectionHelper.updateEpisodeList(
                podcast = podcast,
                oldEpisodes = oldEpisodes,
                newEpisodes = podcastWithAllEpisodes.episodes
    ...
    private fun setPodcastImage(context: Context, tempFileUri: Uri, remoteFileLocation: String) {
        CoroutineScope(IO).launch {
            ...
            if (podcastData != null) {
                val smallCover: String = FileHelper.saveCover(
                    context,
                    podcastData.name,
                    tempFileUri.toString(),
                    Keys.SIZE_COVER_PODCAST_CARD,
                    Keys.PODCAST_SMALL_COVER_FILE
                ).toString()
                val cover: String = FileHelper.saveCover(
                    context,
                    podcastData.name,
                    tempFileUri.toString(),
                    Keys.SIZE_COVER_MAXIMUM,
                    Keys.PODCAST_COVER_FILE
                ).toString()
    ...
    private fun setEpisodeMediaUri(context: Context, tempFileUri: Uri, remoteAudioFileLocation: String) {
        CoroutineScope(IO).launch {
            ...
            if (episode != null) {
                val audio: String = FileHelper.saveCopyOfFile(
                    context,
                    episode.podcastName,
                    tempFileUri,
                    Keys.FILE_TYPE_AUDIO,
                    FileHelper.getFileName(context, tempFileUri),
                    async = true
                ).toString()
                ...
                if (podcast != null) {
                    ...
                    val updatedEpisodes: List<Episode> =
                        CollectionHelper.deleteUnneededAudioFiles(podcast)
    ...
    private fun readPodcastFeed(context: Context, localFileUri: Uri, remoteFileLocation: String) {
        CoroutineScope(IO).launch {
            LogHelper.v(
                TAG,
                "Reading podcast RSS file ($remoteFileLocation) - Thread: ${Thread.currentThread().name}"
            )
            ...
            // validate podcast
            when (CollectionHelper.validateRssPodcast(
                rssPodcast.remoteImageFileLocation.isEmpty(),
                rssPodcast.episodes.isEmpty()
            )) {
    ...
    private fun determineAllowedNetworkTypes(type: Int, ignoreWifiRestriction: Boolean): Int {
        ...
        if (type == Keys.FILE_TYPE_AUDIO) {
            val downloadOverMobileAllowed: Boolean =
                PreferencesHelper.loadEpisodeDownloadOverMobile()
